...
long readBMP(char* filename, unsigned char** imgData, int& width, int& height)
{
    ...
    dataSize = infoheader.biWidth * infoheader.biHeight * 3;
    infoheader_data = (unsigned char*)malloc(dataSize);
...
GLuint createTextureFromBMP(char* filename)
{
    ...
    gluBuild2DMipmaps(GL_TEXTURE_2D, 3, imgWidth, imgHeight, GL_RGB, GL_UNSIGNED_BYTE, imgData);

    free(imgData); // Free the memory we used to load the texture
